def generate_list(file_path):
    '''
        To return the list of list's  which are containing file data.'''

    data_list=None   #local variable    
    try:
        file_obj = open(file_path,'r')
        try:
            gen = (line.split(',') for line in file_obj)  #This is a generator for to generate one line each time until end of file
            for j,line in enumerate(gen):
                if not data_list:
                    #if data list is none then auto create list containing n empty lists, where n will be number of columns.
                    data_list = [[] for i in range(len(line))]
                    if line[-1].find('\n'):
                        line[-1] = line[-1][:-1] #to remove last list element's ('\n') character

                #"This loop is for to convert numbers from string to float, and leave others as strings only"
...
def generate_result(file_path):
    '''
        To return the list of tuples which contain(max price, year, month,company name).
    '''
    ...
    if data_list:
        for i,d in enumerate(data_list):
            if i >= 2:
                m = max(data_list[i][1:])      #This will give max_price for the company
...
if __name__ == '__main__':
    file_path = 'C:\Users\chandan.varma\Downloads\GlobalTest-master\GlobalTest-master\GlobalTest/data2.csv'
